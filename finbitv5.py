...
def analizar_ticker_1d(nombre, symbol, exchange, capital, riesgo_pct, rr_min,
    ...
    # 1D siempre se obtiene (es la base)
    ...
    # 1H y 1W solo si score_1d >= 5 (ahorra créditos)
    if score_1d >= 5:
...
def correr_scanner(tc, capital, riesgo_pct, rr_min, tickers_extra: dict | None = None):
    """
    Scanner: TODOS los tickers únicos — SCANNER_TICKERS + DB + extra.
    Limpia el cache al inicio para datos frescos.
    """
    global _TD_CACHE
    _TD_CACHE = {}  # datos frescos en cada corrida

    port_map = {p["ticker"]:p["titulos"] for p in get_portafolio()}
    tickers_db = get_tickers_db()
    combinados: dict = {}
    combinados.update(SCANNER_TICKERS)   # 1. base del código
    combinados.update(tickers_db)        # 2. DB (persisten entre corridas)
    if tickers_extra:
        combinados.update(tickers_extra) # 3. extra de esta corrida
    ...
    for nombre,(symbol,exchange) in combinados.items():
        ...
        # Detectar origen: BMV = MX, resto = USA
