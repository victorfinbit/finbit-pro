...
# NOTA: exchanges vacíos "" funcionan mejor con TwelveData para acciones USA comunes
SCANNER_TICKERS = {
    "SOXL":("SOXL",""), "TQQQ":("TQQQ",""),
    "NVDA":("NVDA",""),
    "TSLA":("TSLA",""),   "AAPL":("AAPL",""),
...
# Universo para el radar automático (análisis 1D)
# Solo los tickers ADICIONALES al scanner, para no consultar doble
_UNIVERSO_EXTRA = {
...
UNIVERSO = {**SCANNER_TICKERS, **_UNIVERSO_EXTRA}  # deduplicado automáticamente
# ═══════════════════════════════════════════════════════════
...
def radar_masivo(tc, capital, riesgo_pct, rr_min, scan_results: list | None = None):
    ...
    Reutiliza resultados del scanner para no gastar créditos doble."""
    port_map  = {p["ticker"]:p["titulos"] for p in get_portafolio()}
    tickers_db = get_tickers_db()
    universo_completo = {**UNIVERSO, **tickers_db}  # DB tiene prioridad

    # Tickers ya analizados en el scanner — no repetir la consulta API
    ya_analizados = {r["nombre"] for r in (scan_results or [])}
    pendientes = {k:v for k,v in universo_completo.items() if k not in ya_analizados}

    resultados = []
    # Primero agregar los que ya analizó el scanner (gratis, sin API call)
    for r in (scan_results or []):
        if r["nombre"] in universo_completo:
            resultados.append({
                "nombre":r["nombre"], "estado":r["estado"],
                "precio_usd":r["precio_usd"], "precio_mxn":r["precio_mxn"],
                "entrada_mxn":r["entrada_mxn"], "stop_mxn":r["stop_mxn"], "obj_mxn":r["obj_mxn"],
                "rsi":r["rsi"], "rr":r["rr"], "macd_ok":r["macd_ok"],
                "ema200_ok":r["ema200_ok"], "score":r["score"],
                "total_criterios":r.get("total_criterios",8),
                "pot_alza":((r["obj_mxn"]-r["precio_mxn"])/r["precio_mxn"]*100) if r["precio_mxn"] else 0,
                "criterios":r.get("criterios",{}), "sizing":r.get("sizing",{}),
                "titulos_cartera":r.get("titulos_cartera",0),
            })

    total = len(pendientes)
    print(f"  Radar: {len(ya_analizados)} reutilizados del scanner, {total} nuevos a analizar...")

    for i,(nombre,(symbol,exchange)) in enumerate(pendientes.items()):
...
if __name__ == "__main__":
    ...
    # Cargar tickers personalizados del buscador (finbit_tickers.json = fallback opcional)
    # La DB (finbit.db) es la fuente principal — add_ticker_db() escribe ahí directamente
    tickers_extra = {}
    if os.path.exists("finbit_tickers.json"):
        try:
            ...
            for t, ex in raw.items():
                ticker_up = t.upper().strip()
                tickers_extra[ticker_up] = (ticker_up, ex or "")
                add_ticker_db(ticker_up, ex or "")  # migrar a DB automáticamente
            print(f"[4b] Tickers migrados de JSON a DB: {list(tickers_extra.keys())}")
            os.rename("finbit_tickers.json", "finbit_tickers.json.bak")  # ya no se necesita
        except Exception as e:
    ...
    # ── Mini servidor HTTP (reemplaza webbrowser.open del archivo estático) ──
    import http.server, threading, urllib.parse

    PORT = 8765
    dashboard_path = os.path.abspath(OUTPUT_FILE)

    class FinbitHandler(http.server.BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            if self.path == "/" or self.path == "/dashboard.html":
                ...
                custom = list(get_tickers_db().keys())
                body = json.dumps({"custom": custom}).encode()
                self.send_response(200)
        ...
        def do_POST(self):
            ...
            if self.path == "/api/tickers/add":
                ticker = body.get("ticker","").upper().strip()
                exchange = body.get("exchange","").strip()
                if ticker:
                    add_ticker_db(ticker, exchange)
                    print(f"\n  ✅ Ticker agregado: {ticker} ({exchange or 'auto'})")
                    print("  ⚡ Corre python finbit.py para analizarlo\n")
                    resp = json.dumps({"status":"ok","ticker":ticker}).encode()
                else:
                    resp = json.dumps({"status":"error","error":"ticker vacío"}).encode()
                self.send_response(200)
                self.send_header("Content-Type","application/json")
                self.end_headers()
                self.wfile.write(resp)
            elif self.path == "/api/tickers/remove":
                ticker = body.get("ticker","").upper().strip()
                if ticker:
                    remove_ticker_db(ticker)
                    print(f"\n  🗑  Ticker eliminado: {ticker}\n")
    ...
    server = http.server.HTTPServer(("127.0.0.1", PORT), FinbitHandler)
    url = f"http://127.0.0.1:{PORT}"
    print(f"  🌐 Servidor corriendo en {url}")
    print(f"  📌 Agregar tickers desde el dashboard → se guardan en DB automáticamente")
    print(f"  🔄 Para actualizar análisis: Ctrl+C → python finbit.py\n")
    webbrowser.open(url)
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        print("\n  Servidor detenido. Corre python finbit.py para actualizar.\n")
        server.server_close()
