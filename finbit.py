...
def _get_cached(symbol: str, interval: str, exchange: str = "") -> list | None:
    """Devuelve datos del cache. Si no están o el batch los marcó None, intenta SerpApi."""
    global _TD_CACHE
    key = f"{symbol.upper()}:{interval}"

    if key not in _TD_CACHE:
        # Cache miss total: ticker no estaba en el batch
        print(f"    [cache miss] {symbol} {interval} — petición individual TD...")
        result = api_timeseries(symbol, interval, 100, exchange)
        if not result:
            print(f"    [cache miss] {symbol} {interval} — TD falló, intentando SerpApi...")
            result = serpapi_timeseries(symbol, interval)
        _TD_CACHE[key] = result

    elif _TD_CACHE[key] is None:
        # Batch marcó este ticker como None (TD falló para él)
        # Solo intentar SerpApi — NO tocar los que ya tienen datos reales
        print(f"    [serpapi fallback] {symbol} {interval}...")
        result = serpapi_timeseries(symbol, interval)
        if result:
            _TD_CACHE[key] = result
        # Si SerpApi también falla, queda None
...
def _precargar_cache_batch(symbols: list, intervals: list = None):
    """
    Precarga el cache dividiendo en chunks de 8 símbolos máximo.
    El plan Basic de TwelveData permite hasta 8 símbolos por batch request.
    Los que fallen en TD quedan None y _get_cached los reintenta via SerpApi.
    """
    global _TD_CACHE
    ...
    CHUNK = 8  # límite del plan Basic de TwelveData

    print(f"  [batch] Precargando {len(syms)} tickers × {len(intervals)} intervalos (chunks de {CHUNK})...")

    for interval in intervals:
        chunks = [syms[i:i+CHUNK] for i in range(0, len(syms), CHUNK)]
        for idx, chunk in enumerate(chunks):
            print(f"  [batch] TD {interval} chunk {idx+1}/{len(chunks)} → {', '.join(chunk)}")
            batch = api_timeseries_batch(chunk, interval, outputsize=100)
            for sym, vals in batch.items():
                _TD_CACHE[f"{sym}:{interval}"] = vals
            # Marcar solo los de ESTE chunk que fallaron como None
            for sym in chunk:
                key = f"{sym}:{interval}"
                if key not in _TD_CACHE:
                    _TD_CACHE[key] = None  # _get_cached intentará SerpApi después
            if len(chunks) > 1:
                time.sleep(3)
        time.sleep(5)
    ...
    print(f"  [batch] TD listo: {con_datos} con datos, {sin_datos} sin datos (irán a SerpApi)")
...
def radar_masivo(tc, capital, riesgo_pct, rr_min, scan_results: list | None = None,
    ...
    for i, (nombre, (symbol, exchange)) in enumerate(universo_completo.items()):
        ...
        estado = setup["estado"]
        score  = tf["score"]
        total_c= tf.get("total_criterios", 9)
        precio = tf["precio"]
        stop   = tf["stop"]
    ...
    for r in scanner:
        ...
        if etf_peligroso:
            ...
            etf_warning = (f'<div style="background:#fff7e6;border:1px solid #ffd591;border-radius:var(--r);'
                           ...
                           f'⚡ <strong>ETF 3x</strong> — Score mín: {min_s}/9. '
