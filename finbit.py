...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS familia_ops (
        id           INTEGER PRIMARY KEY AUTOINCREMENT,
        usuario      TEXT NOT NULL,
        accion       TEXT NOT NULL,
        nacionalidad TEXT DEFAULT 'Estadounidense',
        tipo         TEXT NOT NULL,
        ...
        fecha        TEXT NOT NULL,
        notas        TEXT,
        fecha_reg    TEXT DEFAULT (datetime('now'))
    );
    """)
    con.commit()
    # Seed usuario Víctor por defecto
    con.execute("INSERT OR IGNORE INTO familia_usuarios (nombre,fecha_inicio) VALUES ('Víctor', date('now'))")
    con.commit()
...
# ══════════════════════════════════════════════════════════
#   MÓDULO FAMILIA — Backend
# ══════════════════════════════════════════════════════════
def get_familia_usuarios() -> list:
    con = sqlite3.connect(DB_FILE); con.row_factory = sqlite3.Row
    rows = con.execute(
        "SELECT * FROM familia_usuarios WHERE activo=1 ORDER BY nombre").fetchall()
    con.close()
    return [dict(r) for r in rows]

def add_familia_usuario(nombre: str, inversion: float = 0,
                         fecha_inicio: str = "", notas: str = ""):
    nombre = nombre.strip().title()
    if not nombre: return {"ok": False, "error": "Nombre vacío"}
    con = sqlite3.connect(DB_FILE)
    try:
        con.execute(
            "INSERT OR IGNORE INTO familia_usuarios "
            "(nombre,inversion_inicial,fecha_inicio,notas) VALUES(?,?,?,?)",
            (nombre, inversion or 0,
             fecha_inicio or datetime.now().strftime("%Y-%m-%d"),
             notas or ""))
        con.commit()
    except Exception as e:
        con.close(); return {"ok": False, "error": str(e)}
    con.close()
    return {"ok": True}

def edit_familia_usuario(uid: int, nombre: str, inversion: float,
                          fecha_inicio: str, notas: str):
    con = sqlite3.connect(DB_FILE)
    con.execute(
        "UPDATE familia_usuarios "
        "SET nombre=?,inversion_inicial=?,fecha_inicio=?,notas=? WHERE id=?",
        (nombre.strip().title(), inversion or 0, fecha_inicio, notas or "", uid))
    con.commit(); con.close()

def del_familia_usuario(uid: int):
    con = sqlite3.connect(DB_FILE)
    con.execute("UPDATE familia_usuarios SET activo=0 WHERE id=?", (uid,))
    con.commit(); con.close()

def get_familia_ops(usuario: str = "") -> list:
    con = sqlite3.connect(DB_FILE); con.row_factory = sqlite3.Row
    if usuario:
        rows = con.execute(
            "SELECT * FROM familia_ops WHERE usuario=? ORDER BY fecha DESC, id DESC",
            (usuario,)).fetchall()
    else:
        rows = con.execute(
            "SELECT * FROM familia_ops ORDER BY fecha DESC, id DESC").fetchall()
    con.close()
    return [dict(r) for r in rows]

def add_familia_op(usuario: str, accion: str, nacionalidad: str,
                    tipo: str, titulos: float, precio: float,
                    impuestos: float, comisiones: float, fecha: str,
                    notas: str = ""):
    imp = impuestos or 0
    com = comisiones or 0
    total = (titulos * precio + imp + com) if tipo == "COMPRA" else (titulos * precio - imp - com)
    con = sqlite3.connect(DB_FILE)
    con.execute(
        "INSERT INTO familia_ops "
        "(usuario,accion,nacionalidad,tipo,titulos,precio,impuestos,comisiones,total,fecha,notas) "
        "VALUES(?,?,?,?,?,?,?,?,?,?,?)",
        (usuario, accion.upper().strip(), nacionalidad, tipo,
         titulos, precio, imp, com, round(total, 4), fecha, notas or ""))
    con.commit(); con.close()

def edit_familia_op(op_id: int, usuario: str, accion: str, nacionalidad: str,
                     tipo: str, titulos: float, precio: float,
                     impuestos: float, comisiones: float, fecha: str, notas: str):
    imp = impuestos or 0
    com = comisiones or 0
    total = (titulos * precio + imp + com) if tipo == "COMPRA" else (titulos * precio - imp - com)
    con = sqlite3.connect(DB_FILE)
    con.execute(
        "UPDATE familia_ops SET usuario=?,accion=?,nacionalidad=?,tipo=?,titulos=?,"
        "precio=?,impuestos=?,comisiones=?,total=?,fecha=?,notas=? WHERE id=?",
        (usuario, accion.upper().strip(), nacionalidad, tipo,
         titulos, precio, imp, com, round(total, 4), fecha, notas or "", op_id))
    con.commit(); con.close()

def del_familia_op(op_id: int):
    con = sqlite3.connect(DB_FILE)
    con.execute("DELETE FROM familia_ops WHERE id=?", (op_id,))
    con.commit(); con.close()

def get_familia_resumen() -> list:
    """Calcula posiciones abiertas y ganancia realizada por usuario (FIFO)."""
    usuarios = get_familia_usuarios()
    ops_all  = get_familia_ops()
    result   = []
    for u in usuarios:
        nombre = u["nombre"]
        u_ops  = [o for o in ops_all if o["usuario"] == nombre]
        # FIFO por accion
        pos: dict = {}
        ganancia_real = 0.0
        for op in sorted(u_ops, key=lambda x: x["fecha"]):
            k = op["accion"]
            if op["tipo"] == "COMPRA":
                if k not in pos:
                    pos[k] = {"titulos": 0.0, "costo_total": 0.0,
                               "nacionalidad": op.get("nacionalidad","—")}
                pos[k]["titulos"]     += op["titulos"]
                pos[k]["costo_total"] += op["total"]
            elif op["tipo"] == "VENTA" and k in pos and pos[k]["titulos"] > 0:
                cto_prom = pos[k]["costo_total"] / pos[k]["titulos"]
                gan = op["total"] - (op["titulos"] * cto_prom)
                ganancia_real += gan
                pos[k]["titulos"]     -= op["titulos"]
                pos[k]["costo_total"] -= op["titulos"] * cto_prom
                if pos[k]["titulos"] <= 0.0001:
                    del pos[k]
        abiertas    = [{"accion": k, **v} for k, v in pos.items()]
        inv_abierta = sum(p["costo_total"] for p in abiertas)
        inv_ini     = u.get("inversion_inicial") or 0
        rend_pct    = (ganancia_real / inv_ini * 100) if inv_ini > 0 else 0
        n_compras   = sum(1 for o in u_ops if o["tipo"] == "COMPRA")
        n_ventas    = sum(1 for o in u_ops if o["tipo"] == "VENTA")
        result.append({
            "usuario":       nombre,
            "id":            u["id"],
            "inversion_ini": round(inv_ini, 2),
            "fecha_inicio":  u.get("fecha_inicio") or "—",
            "ganancia_real": round(ganancia_real, 2),
            "inv_abierta":   round(inv_abierta, 2),
            "rend_pct":      round(rend_pct, 2),
            "n_ops":         len(u_ops),
            "n_compras":     n_compras,
            "n_ventas":      n_ventas,
            "posiciones":    abiertas,
...
<div class="nav"><div class="nav-inner">
  ...
  <button class="nb" onclick="showTab('familia',this)">👥 Familia</button>
</div></div>
...
<div id="tab-familia" class="tab">
  <div style="padding:20px 0 14px">
    <h2 style="font-size:20px;font-weight:600;letter-spacing:-.4px">👥 Familia — Registro de Inversiones</h2>
    <p class="hint">Controla las operaciones de cada inversor: compras, ventas, posiciones abiertas y rendimiento.</p>
  </div>

  <!-- ── RESUMEN CARDS ──────────────────────────────────── -->
  <div id="fam_resumen_cards" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:12px;margin-bottom:20px"></div>

  <!-- ── GESTIÓN DE INVERSORES ─────────────────────────── -->
  <div class="tw" style="margin-bottom:16px">
    <div class="tw-head">
      <span>👤 Inversores</span>
      <button class="btn" onclick="famShowAddUser()" style="font-size:12px">+ Nuevo inversor</button>
    </div>
    <div id="fam_user_list" style="padding:12px 16px;display:flex;gap:8px;flex-wrap:wrap">
      <span class="hint">Cargando...</span>
    </div>
    <!-- Formulario nuevo inversor (oculto por defecto) -->
    <div id="fam_add_user_form" style="display:none;padding:12px 16px;border-top:1px solid var(--brd);background:var(--surface2)">
      <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end">
        <div>
          <div class="hint" style="margin-bottom:4px">Nombre *</div>
          <input id="fau_nombre" type="text" placeholder="Ej: Suegro, Amor..."
            style="border:1px solid var(--brd);border-radius:var(--r);padding:7px 10px;font-size:13px;width:160px;background:var(--surface)">
        </div>
        <div>
          <div class="hint" style="margin-bottom:4px">Inversión inicial MXN</div>
          <input id="fau_inversion" type="number" placeholder="0.00" min="0" step="0.01"
            style="border:1px solid var(--brd);border-radius:var(--r);padding:7px 10px;font-size:13px;width:140px;background:var(--surface)">
        </div>
        <div>
          <div class="hint" style="margin-bottom:4px">Fecha inicio</div>
          <input id="fau_fecha" type="date"
            style="border:1px solid var(--brd);border-radius:var(--r);padding:7px 10px;font-size:13px;background:var(--surface)">
        </div>
        <div>
          <div class="hint" style="margin-bottom:4px">Notas</div>
          <input id="fau_notas" type="text" placeholder="Opcional"
            style="border:1px solid var(--brd);border-radius:var(--r);padding:7px 10px;font-size:13px;width:180px;background:var(--surface)">
        </div>
        <button class="btn" onclick="famGuardarUser()" style="background:var(--green);border-color:var(--green)">Guardar</button>
        <button class="btn" onclick="famCancelAddUser()" style="background:var(--surface);color:var(--text)">Cancelar</button>
      </div>
      <div id="fau_msg" style="margin-top:6px;font-size:12px"></div>
    </div>
  </div>

  <!-- ── REGISTRAR OPERACIÓN ────────────────────────────── -->
  <div class="tw" style="margin-bottom:16px">
    <div class="tw-head"><span>📝 Registrar operación</span></div>
    <div style="padding:14px 16px">
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:10px;margin-bottom:12px">
        <div>
          <div class="hint" style="margin-bottom:4px">Inversor *</div>
          <select id="fop_usuario" style="width:100%;border:1px solid var(--brd);border-radius:var(--r);padding:7px 10px;font-size:13px;background:var(--surface)">
            <option value="">— Selecciona —</option>
          </select>
        </div>
        <div>
          <div class="hint" style="margin-bottom:4px">Acción / ETF *</div>
          <input id="fop_accion" type="text" placeholder="Ej: SOXL, NVDA..."
            style="width:100%;border:1px solid var(--brd);border-radius:var(--r);padding:7px 10px;font-size:13px;background:var(--surface)"
            oninput="this.value=this.value.toUpperCase()">
        </div>
        <div>
          <div class="hint" style="margin-bottom:4px">Tipo *</div>
          <select id="fop_tipo" style="width:100%;border:1px solid var(--brd);border-radius:var(--r);padding:7px 10px;font-size:13px;background:var(--surface)" onchange="famPreview()">
            <option value="COMPRA">COMPRA</option>
            <option value="VENTA">VENTA</option>
          </select>
        </div>
        <div>
          <div class="hint" style="margin-bottom:4px">Nacionalidad</div>
          <select id="fop_nac" style="width:100%;border:1px solid var(--brd);border-radius:var(--r);padding:7px 10px;font-size:13px;background:var(--surface)">
            <option value="Estadounidense">Estadounidense</option>
            <option value="Mexicana">Mexicana</option>
          </select>
        </div>
        <div>
          <div class="hint" style="margin-bottom:4px">Títulos *</div>
          <input id="fop_titulos" type="number" placeholder="0" min="0.0001" step="0.0001"
            style="width:100%;border:1px solid var(--brd);border-radius:var(--r);padding:7px 10px;font-size:13px;background:var(--surface)"
            oninput="famPreview()">
        </div>
        <div>
          <div class="hint" style="margin-bottom:4px">Precio por título MXN *</div>
          <input id="fop_precio" type="number" placeholder="0.00" min="0" step="0.01"
            style="width:100%;border:1px solid var(--brd);border-radius:var(--r);padding:7px 10px;font-size:13px;background:var(--surface)"
            oninput="famPreview()">
        </div>
        <div>
          <div class="hint" style="margin-bottom:4px">Impuestos MXN</div>
          <input id="fop_imp" type="number" placeholder="0.00" min="0" step="0.01"
            style="width:100%;border:1px solid var(--brd);border-radius:var(--r);padding:7px 10px;font-size:13px;background:var(--surface)"
            oninput="famPreview()">
        </div>
        <div>
          <div class="hint" style="margin-bottom:4px">Comisiones MXN</div>
          <input id="fop_com" type="number" placeholder="0.00" min="0" step="0.01"
            style="width:100%;border:1px solid var(--brd);border-radius:var(--r);padding:7px 10px;font-size:13px;background:var(--surface)"
            oninput="famPreview()">
        </div>
        <div>
          <div class="hint" style="margin-bottom:4px">Fecha *</div>
          <input id="fop_fecha" type="date"
            style="width:100%;border:1px solid var(--brd);border-radius:var(--r);padding:7px 10px;font-size:13px;background:var(--surface)">
        </div>
        <div style="grid-column:1/-1">
          <div class="hint" style="margin-bottom:4px">Notas</div>
          <input id="fop_notas" type="text" placeholder="Opcional — contexto de la operación"
            style="width:100%;border:1px solid var(--brd);border-radius:var(--r);padding:7px 10px;font-size:13px;background:var(--surface)">
        </div>
      </div>
      <!-- Preview -->
      <div id="fop_preview" style="display:none;background:var(--surface2);border:1px solid var(--brd);border-radius:var(--r);padding:10px 14px;margin-bottom:10px;font-size:13px"></div>
      <div style="display:flex;gap:8px;align-items:center">
        <button class="btn" onclick="famRegistrarOp()" style="background:var(--green);border-color:var(--green);font-size:13px;padding:8px 20px">✅ Registrar operación</button>
        <span id="fop_msg" style="font-size:12px"></span>
      </div>
    </div>
  </div>
  ...
  <div class="tw">
    <div class="tw-head">
      <span id="fam_hist_title">Historial de operaciones</span>
      <div style="display:flex;gap:8px;align-items:center">
        <select id="fam_fil_user" onchange="famFiltrarHist()"
          style="padding:4px 8px;border:1px solid var(--brd);border-radius:6px;font-size:12px;background:var(--surface)">
          <option value="">Todos los inversores</option>
        </select>
        <select id="fam_fil_tipo" onchange="famFiltrarHist()"
          style="padding:4px 8px;border:1px solid var(--brd);border-radius:6px;font-size:12px;background:var(--surface)">
          <option value="">Compras y ventas</option>
          <option value="COMPRA">Solo compras</option>
          <option value="VENTA">Solo ventas</option>
        </select>
        <input id="fam_fil_accion" type="text" placeholder="🔍 Acción..."
          style="padding:4px 8px;border:1px solid var(--brd);border-radius:6px;font-size:12px;background:var(--surface);width:110px"
          oninput="famFiltrarHist()">
      </div>
    </div>
    <div style="overflow-x:auto">
      <table id="fam_hist_table">
        <thead><tr>
          <th>Inversor</th><th>Acción</th><th>Nac.</th><th>Tipo</th>
          <th>Títulos</th><th>Precio</th><th>Imp.</th><th>Com.</th>
          <th>Total MXN</th><th>Fecha</th><th>Notas</th><th>Acciones</th>
        </tr></thead>
        <tbody id="fam_hist_body">
          <tr><td colspan="12" style="text-align:center;color:var(--muted);padding:28px">Cargando...</td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
function famFmtPct(n) {{
  if (n == null || isNaN(n)) return '—';
  return (n >= 0 ? '+' : '') + n.toFixed(2) + '%';
}}

// ── Cargar datos del servidor ─────────────────────────────
async function famCargar() {{
  try {{
    const [uRes, rRes, oRes] = await Promise.all([
      fetch('/api/familia/usuarios').then(r => r.json()),
      fetch('/api/familia/resumen').then(r => r.json()),
      fetch('/api/familia/ops').then(r => r.json()),
    ]);
    _famUsuarios = uRes;
    _famOps      = oRes;
    famRenderResumen(rRes);
    famRenderUserList(uRes);
    famRenderHistorial(oRes);
    famPoblarSelects(uRes);
  }} catch(e) {{
    console.error('famCargar error:', e);
  }}
}}

// ── Cards de resumen por inversor ─────────────────────────
function famRenderResumen(resumen) {{
  const cont = document.getElementById('fam_resumen_cards');
  if (!cont) return;
  if (!resumen.length) {{
    cont.innerHTML = '<p class="hint">Agrega un inversor para ver el resumen.</p>';
    return;
  }}
  const ganTotal   = resumen.reduce((s,r) => s + r.ganancia_real, 0);
  const invTotal   = resumen.reduce((s,r) => s + r.inv_abierta, 0);
  const ganColor   = ganTotal >= 0 ? 'var(--green)' : 'var(--red)';

  // Card global
  let html = `<div style="background:var(--surface);border:2px solid var(--brd2);border-radius:var(--r2);padding:16px 18px;grid-column:1/-1">
    <div style="font-size:10px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);margin-bottom:10px;font-weight:600">Resumen global</div>
    <div style="display:flex;gap:24px;flex-wrap:wrap;align-items:center">
      <div><div class="hint" style="font-size:10px">Ganancia realizada</div>
        <div style="font-size:22px;font-weight:600;color:${{ganColor}};font-family:var(--mono)">${{famFmt(ganTotal)}}</div></div>
      <div><div class="hint" style="font-size:10px">Capital en posiciones</div>
        <div style="font-size:18px;font-weight:500;font-family:var(--mono)">${{famFmt(invTotal)}}</div></div>
      <div><div class="hint" style="font-size:10px">Inversores activos</div>
        <div style="font-size:18px;font-weight:500">${{resumen.length}}</div></div>
      <div><div class="hint" style="font-size:10px">Total operaciones</div>
        <div style="font-size:18px;font-weight:500">${{resumen.reduce((s,r)=>s+r.n_ops,0)}}</div></div>
    </div>
  </div>`;

  // Card por inversor
  for (const r of resumen) {{
    const gCol = r.ganancia_real >= 0 ? 'var(--green)' : 'var(--red)';
    const rCol = r.rend_pct >= 0 ? 'var(--green)' : 'var(--red)';
    let posHtml = '';
    if (r.posiciones && r.posiciones.length) {{
      posHtml = '<div style="margin-top:10px;border-top:1px solid var(--brd);padding-top:8px">'
        + '<div class="hint" style="font-size:10px;margin-bottom:6px">POSICIONES ABIERTAS</div>';
      for (const p of r.posiciones) {{
        posHtml += `<div style="display:flex;justify-content:space-between;font-size:11px;padding:2px 0">
          <span><strong>${{p.accion}}</strong> <span class="hint">${{p.titulos.toFixed(4)}} tít</span></span>
          <span style="font-family:var(--mono)">${{famFmt(p.costo_total)}}</span></div>`;
      }}
      posHtml += '</div>';
    }} else {{
      posHtml = '<div style="margin-top:8px;font-size:11px;color:var(--muted)">Sin posiciones abiertas</div>';
    }}
    html += `<div style="background:var(--surface);border:1px solid var(--brd);border-radius:var(--r2);padding:16px 18px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
        <div style="font-size:15px;font-weight:600">${{r.usuario}}</div>
        <div style="display:flex;gap:4px">
          <button class="btn-sm btn-edit" onclick="famEditUser(${{r.id}},'${{r.usuario}}',${{r.inversion_ini}},'${{r.fecha_inicio}}','${{r.notas}}')">Editar</button>
          <button class="btn-sm btn-del" onclick="famDelUser(${{r.id}},'${{r.usuario}}')">Borrar</button>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:12px">
        <div><div class="hint" style="font-size:10px">Inv. inicial</div>
          <div style="font-family:var(--mono);font-weight:500">${{famFmt(r.inversion_ini)}}</div></div>
        <div><div class="hint" style="font-size:10px">Ganancia real</div>
          <div style="font-family:var(--mono);font-weight:600;color:${{gCol}}">${{famFmt(r.ganancia_real)}}</div></div>
        <div><div class="hint" style="font-size:10px">Rendimiento</div>
          <div style="font-weight:600;color:${{rCol}}">${{famFmtPct(r.rend_pct)}}</div></div>
        <div><div class="hint" style="font-size:10px">Ops: ${{r.n_compras}}C / ${{r.n_ventas}}V</div>
          <div style="color:var(--muted)">Desde: ${{r.fecha_inicio || '—'}}</div></div>
      </div>
      ${{posHtml}}
    </div>`;
  }}
...
// ── Lista de inversores ───────────────────────────────────
function famRenderUserList(usuarios) {{
  const el = document.getElementById('fam_user_list');
  if (!el) return;
  if (!usuarios.length) {{
    el.innerHTML = '<span class="hint">No hay inversores. Agrega el primero.</span>';
    return;
  }}
  el.innerHTML = usuarios.map(u =>
    `<span class="ticker-chip"><strong>${{u.nombre}}</strong>`+
    `<span class="hint" style="margin-left:4px">ini: ${{famFmt(u.inversion_inicial)}}</span></span>`
  ).join('');
}}

// ── Historial de operaciones ──────────────────────────────
function famRenderHistorial(ops) {{
  const tbody = document.getElementById('fam_hist_body');
  if (!tbody) return;
  if (!ops.length) {{
    tbody.innerHTML = '<tr><td colspan="12" style="text-align:center;color:var(--muted);padding:24px">Sin operaciones registradas</td></tr>';
    return;
  }}
  tbody.innerHTML = ops.map(o => {{
    const tc = o.tipo === 'COMPRA'
      ? '<span style="color:var(--green);font-weight:600">↑ COMPRA</span>'
      : '<span style="color:var(--red);font-weight:600">↓ VENTA</span>';
    const totCol = o.tipo === 'COMPRA' ? 'var(--text)' : 'var(--green)';
    return `<tr class="datarow" data-usuario="${{o.usuario}}" data-tipo="${{o.tipo}}" data-accion="${{o.accion}}">
      <td><strong>${{o.usuario}}</strong></td>
      <td><strong>${{o.accion}}</strong></td>
      <td style="font-size:11px;color:var(--muted)">${{o.nacionalidad||'—'}}</td>
      <td>${{tc}}</td>
      <td class="num">${{o.titulos}}</td>
      <td class="num">${{famFmt(o.precio)}}</td>
      <td class="num" style="color:var(--muted)">${{o.impuestos?famFmt(o.impuestos):'—'}}</td>
      <td class="num" style="color:var(--muted)">${{o.comisiones?famFmt(o.comisiones):'—'}}</td>
      <td class="num" style="color:${{totCol}};font-weight:600">${{famFmt(o.total)}}</td>
      <td style="font-size:11px;font-family:var(--mono)">${{o.fecha||'—'}}</td>
      <td style="font-size:11px;color:var(--muted)">${{o.notas||'—'}}</td>
      <td>
        <button class="btn-sm btn-edit" onclick="famEditOp(${{o.id}})">Editar</button>
        <button class="btn-sm btn-del" onclick="famDelOp(${{o.id}},'${{o.accion}}')">Borrar</button>
      </td>
    </tr>`;
  }}).join('');
}}

// ── Poblar selects de usuario ─────────────────────────────
function famPoblarSelects(usuarios) {{
  ['fop_usuario','fam_fil_user'].forEach(id => {{
    const el = document.getElementById(id);
    if (!el) return;
    const def = id === 'fam_fil_user' ? '<option value="">Todos los inversores</option>'
                                       : '<option value="">— Selecciona —</option>';
    el.innerHTML = def + usuarios.map(u =>
...
// ── Filtrar historial ─────────────────────────────────────
function famFiltrarHist() {{
  const usr  = (document.getElementById('fam_fil_user')?.value || '').toLowerCase();
  const tipo = document.getElementById('fam_fil_tipo')?.value || '';
  const acc  = (document.getElementById('fam_fil_accion')?.value || '').toUpperCase();
  let shown = 0;
  document.querySelectorAll('#fam_hist_body tr').forEach(tr => {{
    const u = (tr.dataset.usuario || '').toLowerCase();
    const t = tr.dataset.tipo || '';
    const a = tr.dataset.accion || '';
    const ok = (!usr || u.includes(usr)) && (!tipo || t === tipo) && (!acc || a.includes(acc));
    tr.style.display = ok ? '' : 'none';
    if (ok) shown++;
  }});
  const title = document.getElementById('fam_hist_title');
  if (title) title.textContent = `Historial (${{shown}} operaciones)`;
}}

// ── Preview de operación ──────────────────────────────────
function famPreview() {{
  const t = parseFloat(document.getElementById('fop_titulos')?.value) || 0;
  const p = parseFloat(document.getElementById('fop_precio')?.value) || 0;
  const imp = parseFloat(document.getElementById('fop_imp')?.value) || 0;
  const com = parseFloat(document.getElementById('fop_com')?.value) || 0;
  const tipo = document.getElementById('fop_tipo')?.value || 'COMPRA';
  const prev = document.getElementById('fop_preview');
  if (!prev) return;
  if (t && p) {{
    const total = tipo === 'COMPRA' ? t*p+imp+com : t*p-imp-com;
    const col = tipo === 'COMPRA' ? 'var(--green)' : 'var(--red)';
    prev.style.display = 'block';
    prev.innerHTML = `<strong style="color:${{col}}">${{tipo}}</strong> ${{t}} tít × ${{famFmt(p)}} = `+
      `<strong style="color:${{col}}">${{famFmt(total)}}</strong> MXN`+
      (imp||com ? ` <span class="hint">(imp ${{famFmt(imp)}} + com ${{famFmt(com)}})</span>` : '');
  }} else {{
...
// ── Registrar operación ───────────────────────────────────
async function famRegistrarOp() {{
  const d = {{
    usuario:     document.getElementById('fop_usuario')?.value,
    accion:      document.getElementById('fop_accion')?.value,
    nacionalidad:document.getElementById('fop_nac')?.value,
    tipo:        document.getElementById('fop_tipo')?.value,
    titulos:     parseFloat(document.getElementById('fop_titulos')?.value),
    precio:      parseFloat(document.getElementById('fop_precio')?.value),
    impuestos:   parseFloat(document.getElementById('fop_imp')?.value) || 0,
    comisiones:  parseFloat(document.getElementById('fop_com')?.value) || 0,
    fecha:       document.getElementById('fop_fecha')?.value,
    notas:       document.getElementById('fop_notas')?.value,
  }};
  const msg = document.getElementById('fop_msg');
  if (!d.usuario || !d.accion || !d.titulos || !d.precio || !d.fecha) {{
    if (msg) msg.innerHTML = '<span style="color:var(--red)">⚠ Completa los campos obligatorios (*)</span>';
    return;
  }}
  try {{
    const r = await fetch('/api/familia/ops/add', {{
      method:'POST', headers:{{'Content-Type':'application/json'}},
      body: JSON.stringify(d)
    }}).then(r => r.json());
    if (r.status === 'ok') {{
      if (msg) msg.innerHTML = '<span style="color:var(--green)">✅ Operación registrada</span>';
      setTimeout(() => {{ if(msg) msg.innerHTML=''; }}, 4000);
      ['fop_accion','fop_titulos','fop_precio','fop_imp','fop_com','fop_notas'].forEach(id => {{
        const el = document.getElementById(id); if(el) el.value='';
      }});
      document.getElementById('fop_preview').style.display='none';
      famCargar();
    }} else {{
      if (msg) msg.innerHTML = `<span style="color:var(--red)">❌ ${{r.error||'Error'}}</span>`;
    }}
  }} catch(e) {{
    if (msg) msg.innerHTML = '<span style="color:var(--red)">❌ Error de conexión</span>';
  }}
}}

// ── Eliminar operación ────────────────────────────────────
async function famDelOp(id, accion) {{
  if (!confirm(`¿Borrar operación de ${{accion}}?`)) return;
  await fetch('/api/familia/ops/del', {{
    method:'POST', headers:{{'Content-Type':'application/json'}},
...
// ── Editar operación (simple prompt por ahora) ────────────
async function famEditOp(id) {{
  const op = _famOps.find(o => o.id === id);
  if (!op) return;
  const nuevoP = prompt(`Precio de "${{op.accion}}" (actual: ${{op.precio}}):`, op.precio);
  if (nuevoP === null) return;
  const nuevoT = prompt(`Títulos (actual: ${{op.titulos}}):`, op.titulos);
  if (nuevoT === null) return;
  await fetch('/api/familia/ops/edit', {{
    method:'POST', headers:{{'Content-Type':'application/json'}},
    body: JSON.stringify({{...op, id, precio: parseFloat(nuevoP), titulos: parseFloat(nuevoT)}})
  }});
  famCargar();
}}

// ── Formulario de usuario ─────────────────────────────────
function famShowAddUser() {{
  document.getElementById('fam_add_user_form').style.display = 'block';
  document.getElementById('fau_nombre').focus();
}}
function famCancelAddUser() {{
  document.getElementById('fam_add_user_form').style.display = 'none';
  ['fau_nombre','fau_inversion','fau_fecha','fau_notas'].forEach(id => {{
    const el = document.getElementById(id); if(el) el.value='';
  }});
}}
async function famGuardarUser() {{
  const d = {{
    nombre:    document.getElementById('fau_nombre')?.value,
    inversion: parseFloat(document.getElementById('fau_inversion')?.value) || 0,
    fecha_inicio: document.getElementById('fau_fecha')?.value,
    notas:     document.getElementById('fau_notas')?.value,
  }};
  const msg = document.getElementById('fau_msg');
  if (!d.nombre.trim()) {{
    if(msg) msg.innerHTML='<span style="color:var(--red)">⚠ Escribe un nombre</span>'; return;
  }}
  const r = await fetch('/api/familia/usuarios/add',{{
    method:'POST', headers:{{'Content-Type':'application/json'}},
    body: JSON.stringify(d)
  }}).then(r=>r.json());
  if (r.status==='ok') {{
...
function famEditUser(id, nombre, inv, fechaIni, notas) {{
  const nuevoNombre = prompt('Nombre:', nombre);
  if (nuevoNombre === null) return;
  const nuevaInv = prompt('Inversión inicial MXN:', inv);
  if (nuevaInv === null) return;
  fetch('/api/familia/usuarios/edit', {{
    method:'POST', headers:{{'Content-Type':'application/json'}},
    body: JSON.stringify({{id, nombre:nuevoNombre, inversion:parseFloat(nuevaInv)||0,
                           fecha_inicio:fechaIni, notas}})
  }}).then(() => famCargar());
}}
async function famDelUser(id, nombre) {{
  if (!confirm(`¿Desactivar a ${{nombre}}? Sus operaciones se conservan.`)) return;
  await fetch('/api/familia/usuarios/del',{{
    method:'POST', headers:{{'Content-Type':'application/json'}},
    body: JSON.stringify({{id}})
  }});
  famCargar();
...
const _famOrigShowTab = window.showTab;
window.showTab = function(tab, btn) {{
  _famOrigShowTab && _famOrigShowTab(tab, btn);
  if (tab === 'familia') famCargar();
}};
</script></body></html>"""
...
# ══════════════════════════════════════════════════════════
#   API FAMILIA — Rutas Flask
# ══════════════════════════════════════════════════════════
@app.route("/api/familia/usuarios")
def api_familia_usuarios():
    try:
        return jsonify(get_familia_usuarios())
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/api/familia/usuarios/add", methods=["POST"])
def api_familia_usuarios_add():
    try:
        d = flask_req.get_json(force=True) or {}
        nombre = (d.get("nombre") or "").strip()
        if not nombre:
            return jsonify({"status": "error", "error": "Nombre vacío"}), 400
        result = add_familia_usuario(
            nombre=nombre,
            inversion=float(d.get("inversion") or 0),
            fecha_inicio=d.get("fecha_inicio") or "",
            notas=d.get("notas") or ""
        )
        return jsonify({"status": "ok"} if result.get("ok") else result)
    except Exception as e:
        return jsonify({"status": "error", "error": str(e)}), 500

@app.route("/api/familia/usuarios/edit", methods=["POST"])
def api_familia_usuarios_edit():
    try:
        d = flask_req.get_json(force=True) or {}
        edit_familia_usuario(
            uid=int(d["id"]),
            nombre=d.get("nombre",""),
            inversion=float(d.get("inversion") or 0),
            fecha_inicio=d.get("fecha_inicio") or "",
...
@app.route("/api/familia/usuarios/del", methods=["POST"])
def api_familia_usuarios_del():
    try:
        d = flask_req.get_json(force=True) or {}
        del_familia_usuario(int(d["id"]))
        return jsonify({"status": "ok"})
    except Exception as e:
        return jsonify({"status": "error", "error": str(e)}), 500

@app.route("/api/familia/ops")
def api_familia_ops_get():
    try:
        usuario = flask_req.args.get("usuario", "")
        return jsonify(get_familia_ops(usuario))
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/api/familia/ops/add", methods=["POST"])
def api_familia_ops_add():
    try:
        d = flask_req.get_json(force=True) or {}
        if not d.get("usuario") or not d.get("accion") or not d.get("titulos") or not d.get("precio"):
            return jsonify({"status": "error", "error": "Faltan campos obligatorios"}), 400
        add_familia_op(
            usuario     = d["usuario"],
            accion      = d["accion"],
            nacionalidad= d.get("nacionalidad","Estadounidense"),
            tipo        = d.get("tipo","COMPRA"),
            titulos     = float(d["titulos"]),
            precio      = float(d["precio"]),
            impuestos   = float(d.get("impuestos") or 0),
            comisiones  = float(d.get("comisiones") or 0),
...
@app.route("/api/familia/ops/edit", methods=["POST"])
def api_familia_ops_edit():
    try:
        d = flask_req.get_json(force=True) or {}
        edit_familia_op(
            op_id       = int(d["id"]),
...
@app.route("/api/familia/ops/del", methods=["POST"])
def api_familia_ops_del():
    try:
        d = flask_req.get_json(force=True) or {}
        del_familia_op(int(d["id"]))
        return jsonify({"status": "ok"})
    except Exception as e:
        return jsonify({"status": "error", "error": str(e)}), 500

@app.route("/api/familia/resumen")
def api_familia_resumen():
    try:
        return jsonify(get_familia_resumen())
    except Exception as e:
        return jsonify({"error": str(e)}), 500
