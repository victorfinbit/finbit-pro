...
def _get_cached(symbol: str, interval: str, exchange: str = "") -> list | None:
    ...
    if key not in _TD_CACHE:
        # Cache miss total: ticker no estaba en el batch (ej. agregado después)
        ...
        # El batch lo marcó como None (TwelveData falló para este ticker)
        # → intentar SerpApi como fallback antes de rendirse
        print(f"    [cache None] {symbol} {interval} — intentando SerpApi fallback...")
        result = serpapi_timeseries(symbol, interval)
        if result:
            _TD_CACHE[key] = result   # actualizar cache con datos de SerpApi
