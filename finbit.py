...
# ── SERPAPI FALLBACK ──────────────────────────────────────
_SERPAPI_WINDOW_MAP = {
    "1day":   "6M",   # 6 meses de datos diarios
    "1h":     "1M",   # 1 mes de datos horarios
    "1week":  "1Y",   # 1 año de datos semanales
}

def serpapi_timeseries(symbol: str, interval: str) -> list | None:
    """
    Obtiene serie histórica desde SerpApi / Google Finance.
    Retorna lista de dicts con 'close' y 'volume' para compatibilidad con ohlcv_to_close.
    Solo se llama cuando TwelveData falla o el ticker está en SERPAPI_TICKERS.
    """
    if not SERPAPI_KEY or SERPAPI_KEY == "TU_SERPAPI_KEY":
        return None
    window = _SERPAPI_WINDOW_MAP.get(interval, "6M")
    # Mapear símbolo a formato Google Finance (SYMBOL:EXCHANGE)
    ...
    try:
        tdb = get_tickers_db()
        if symbol.upper() in tdb:
            _, db_exchange = tdb[symbol.upper()]
            exchange = db_exchange or _exchange_map.get(symbol.upper(), "NASDAQ")
        else:
...
def correr_scanner(tc, capital, riesgo_pct, rr_min, tickers_extra: dict | None = None,
    ...
    for nombre,(symbol,exchange) in combinados.items():
        ...
        # Score ajustado por macro (penalización VIX)
        score_ajustado = max(0, score - regimen["penalizacion"])

        # Score mínimo según tipo de activo y VIX
        min_score = score_minimo_entrada(nombre, vix)
        etf_peligroso = es_etf_apalancado(nombre)

        # Detectar EXIT
        exit_info = detectar_exit(nombre, tf_1d, score)

        # Estado con filtros macro aplicados
        if explosion and score_ajustado >= 8:
            estado = "ROCKET"
        elif score_ajustado >= min_score and c.get("emas") and c.get("ema200") and c.get("macd"):
            estado = "BUY"
        elif regimen["color"] == "rojo" and score_ajustado < min_score:
            estado = "BLOQUEADO"   # el mercado no permite entrada
        ...
        # Si hay EXIT, sobreescribir estado (prioridad máxima para posiciones abiertas)
        if exit_info["exit"] and tit > 0:
            estado = "EXIT"

        # Guardar en historial para detectar caídas futuras
        try:
    ...
    orden = {"ROCKET":0,"BUY":1,"EXIT":2,"WATCH":3,"SKIP":4,"SHORT":5,"BLOQUEADO":6}
...
def radar_masivo(tc, capital, riesgo_pct, rr_min, scan_results: list | None = None):
    """Radar: analiza UNIVERSO + DB en modo 1D. Reutiliza cache del scanner si scan_results!=None."""
    port_map  = {p["ticker"]:p["titulos"] for p in get_portafolio()}
    tickers_db = get_tickers_db()
    ...
    for i,(nombre,(symbol,exchange)) in enumerate(universo_completo.items()):
        ...
        values = _get_cached(symbol, "1day", exchange)  # cache — 0 calls extra si ya analizó en scanner
...
def badge_estado(s):
    m={"ROCKET":("b-rocket","🚀 Explosión"),"BUY":("b-buy","↑ Compra"),
       ...
       "EXIT":("b-exit","⚠️ EXIT YA"),"BLOQUEADO":("b-blocked","🔒 Bloqueado VIX")}
    c,t=m.get(s,("b-none",s)); return badge(c,t)
...
def render_scan_rows(scanner, tc):
    ...
    for r in scanner:
        ...
        adx_val   = r.get("adx", 0)
        regimen_r = r.get("regimen", {})
        penaliz   = regimen_r.get("penalizacion", 0)
        ...
        if etf_peligroso:
            ...
            etf_warning = (f'<div style="background:#fff7e6;border:1px solid #ffd591;border-radius:var(--r);'
                           ...
                           f'⚡ <strong>ETF apalancado 3x</strong> — volatility decay activo. '
                           f'Score mínimo para entrada: <strong>{min_s}/9</strong>. '
                           f'Solo comprar cuando VIX &lt; 20 y S&P500 alcista.</div>')
        ...
        detail=(f'<div class="detail-panel">'
                ...
                f'<div class="dp-sec"><div class="dp-sec-t">Score técnico</div>{score_block}{score_adj_note}</div>'
        ...
        h+=(f'<tr class="datarow" onclick="toggle(\'{rid}\')">'
            f'<td><strong>{r["nombre"]}</strong>{etf_badge}{cartera_badge}</td>'
...
<div id="tab-scanner" class="tab">
  ...
  <!-- Guía de score -->
  <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:14px;align-items:center;padding:10px 0">
    <span style="font-size:11px;color:var(--muted);font-weight:600">Guía de score:</span>
    <span style="background:#fef2f2;border:1px solid #fecaca;color:#dc2626;border-radius:6px;padding:4px 11px;font-size:11px;font-weight:600">≤3 🚫 Prohibido comprar</span>
    <span style="background:#fffbeb;border:1px solid #fde68a;color:#b45309;border-radius:6px;padding:4px 11px;font-size:11px;font-weight:600">4–5 👁 Solo vigilar</span>
    <span style="background:#f0fdf4;border:1px solid #bbf7d0;color:#16a34a;border-radius:6px;padding:4px 11px;font-size:11px;font-weight:600">6 ✅ Posible entrada</span>
    <span style="background:#eff6ff;border:1px solid #bfdbfe;color:#1d4ed8;border-radius:6px;padding:4px 11px;font-size:11px;font-weight:600">≥7 🚀 Entrada fuerte</span>
  </div>
